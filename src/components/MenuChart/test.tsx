...
const years = ['Area 1', 'Area 2', 'Area 3', 'Area 4']
const data = [
	{
		type: 'Feature',
		properties: {
...
describe('<MenuChart />', () => {
	it('should render the menu with default strings', () => {
		renderWithTheme(<MenuChart years={years} data={data} />)
		expect(screen.getByText(/Area 1/i)).toBeInTheDocument()
		expect(screen.getByText(/Area 2/i)).toBeInTheDocument()
		expect(screen.getByText(/Area 3/i)).toBeInTheDocument()
		expect(screen.getByText(/Area 4/i)).toBeInTheDocument()
	})

	it('should render the menu with default style', () => {
		renderWithTheme(<MenuChart years={years} data={data} />)
		expect(screen.getByText(/Area 1/i)).toHaveStyle({
			background: 'none'
		})
	})
})
